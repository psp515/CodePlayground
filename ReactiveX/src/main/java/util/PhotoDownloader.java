...
public class PhotoDownloader {
    ...
    public Observable<Photo> searchForPhotos(String searchQuery) throws IOException, InterruptedException {
        return Observable.defer(() -> {
            try {
                var photos = DuckDuckGoDriver.searchForImages(searchQuery);

                return Observable.fromIterable(photos)
                        .subscribeOn(Schedulers.io())
                        .map(this::getPhoto)
                        .onErrorResumeNext(e -> {
                            log.log(Level.WARNING, "Failed to download photo: " + e);
                            return Observable.empty(); // Continue with the next photo
                        });
            } catch (InterruptedException | IOException e) {
                log.log(Level.WARNING, "Error searching for photos: " + e);
